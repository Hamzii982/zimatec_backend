...
<li class="list-group-item d-flex align-items-center gap-2 px-0">
    <form method="POST" action="{{ route('admin.workflow.steps.update', $step) }}" class="d-flex flex-grow-1 gap-2 align-items-center">
        @csrf @method('PUT')
        <input type="text" name="name" class="form-control form-control-sm flex-grow-1" value="{{ $step->name }}" required>
        <input type="number" name="order_index" class="form-control form-control-sm" style="width: 80px;" value="{{ $step->order_index }}">
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="is_required" value="1" id="req-{{ $step->id }}" @checked($step->is_required)>
            <label class="form-check-label small" for="req-{{ $step->id }}">P</label>
        </div>

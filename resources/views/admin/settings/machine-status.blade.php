...
<div class="zt-compare container mt-4">
    <div class="card shadow-sm zt-card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Machine Statuses</h5>
            <a href="{{ route('admin.settings.machine-status.show') }}" class="zt-export-btn">
                <i class="bi bi-plus-circle"></i> Neue Machinenstatus
            </a>
        </div>
                            ...
                            <tr id="row-{{ $status->id }}">
                                ...
                                <td>
                                    <div class="d-flex gap-2">
                                        <a href="{{ route('admin.settings.machine-status.show', $status->id) }}" class="zt-icon-btn zt-icon-btn--edit">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <form action="{{ route('admin.settings.machine-status.delete', $status->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
...
document.querySelectorAll('.toggle-active').forEach((checkbox) => {
    checkbox.addEventListener('change', function() {
        ...
        fetch(`/admin/settings/machine-status/toggle/${id}`, {
            method: 'PATCH',
            headers: {

...
@section('content')
    <div class="container mt-4 zt-compare">
        <div class="card zt-card shadow-sm">
            <div class="card-header d-flex justify-content-between align-items-center py-2">
                <h5 class="mb-0">Neues Projekt erstellen</h5>
            ...
            <div class="card-body p-3">
                ...
                {{-- Mini stepper --}}
                <div class="d-flex justify-content-between align-items-center mb-3 px-2 zt-stepper">
                    <div class="zt-stepper-step text-center flex-fill">
                        <i class="bi bi-pencil-square"></i>
                        <div class="zt-step-label">Details eingeben</div>
                    </div>
                    <div class="zt-stepper-line"></div>
                    <div class="zt-stepper-step text-center flex-fill">
                        <i class="bi bi-diagram-3"></i>
                        <div class="zt-step-label">Workflow zuordnen</div>
                    </div>
                    <div class="zt-stepper-line"></div>
                    <div class="zt-stepper-step text-center flex-fill">
                        <i class="bi bi-folder2-open"></i>
                        <div class="zt-step-label">Ordnerstruktur wird erstellt</div>
                    </div>
                </div>

                @php
                    $selectedStatusId = old('project_status_id', isset($project) ? $project->project_status_id : null);
                @endphp
                ...
                <form action="{{ route('admin.projects.store') }}" method="POST">
                            ...
                            <div class="row g-2">
                                <div class="col-6">
                                    <input type="text" name="auftragsnummer_zt" id="auftragsnummer_zt"
                                           ...
                                           placeholder="ZimaTec" required>
                                </div>
                                <div class="col-6">
                                    <input type="text" name="auftragsnummer_zf" id="auftragsnummer_zf"
                                           class="form-control zt-select form-control-sm at-least-one"
                                           placeholder="Zimmermann Formtechnik" required>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="zt-status-group">
                                @foreach($statuses as $status)
                                    <input type="radio" class="btn-check" name="project_status_id"
                                           ...
                                           autocomplete="off"
                                           {{ (!is_null($selectedStatusId) ? $selectedStatusId == $status->id : strtolower(trim($status->name)) === 'neue') ? 'checked' : '' }}
                                           required>
                    ...
                    <div class="row mb-2">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="project_name" id="project_name" class="form-control zt-select form-control-sm" placeholder="Projektname" required>
                        </div>
                        <div class="col-md-6">
                            <label for="kunde" class="zt-form-label">Kundenname</label>
                            <input type="text" name="kunde" id="kunde" class="form-control zt-select form-control-sm" placeholder="Kundenname">
                        </div>
                    </div>
                    ...
                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" role="switch"
                               id="attach_to_workflow" name="attach_to_workflow" value="1">
                    ...
                    <div class="text-end">
                        <button type="submit" class="zt-btn zt-btn--primary">Projekt Erstellen</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <style>
        ...
        .zt-stepper-step i { font-size: 1.1rem; color: var(--zt-muted); }
        .zt-step-label { font-size: .72rem; color: var(--zt-muted); margin-top: 2px; }
        .zt-stepper-line { flex: 0 0 40px; height: 1px; background: var(--zt-line); margin: 0 4px; align-self: center; }
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            inputs.forEach(input => {
                input.addEventListener('input', function () {
                    ...
                    inputs.forEach(el => { el.required = !oneIsFilled; });
                });
            });
